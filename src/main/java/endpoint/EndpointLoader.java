...
public final class EndpointLoader {
	...
	private void scanEndpoints() {
		...
		for (Class<?> endpoint : clazzes) {
			endpoints.put(endpoint, new EndpointRef<>(endpoint));
		}
	}
	...
	private Collection<EndpointRef<?>> generateEndpointsMap() {
		scanEndpoints();
		scanActions();
		scanTransformers();
		if (enableHooks) {
	...
	private void addTransformerForObject(Class<?> objectClazz, Class<? extends Transformer> transformerClazz) {
		for (Method method : transformerClazz.getDeclaredMethods()) {
			for (EndpointRef<?> endpoint : getEndpoints(objectClazz, transformerClazz.getSimpleName())) {
	...
	private <T> List<EndpointRef<? extends T>> getEndpoints(Class<T> objectClazz, String featureClazz) {
		List<EndpointRef<? extends T>> list = new ArrayList<>();
		for (Class<?> endpoint : endpoints.keySet()) {
			if (objectClazz.isAssignableFrom(endpoint)) {
				list.add((EndpointRef<T>) endpoints.get(endpoint));
			}
		}
		if (list.isEmpty()) {
	...
	private void addAction(Class<?> objectClazz, Method method) {
		...
		for (EndpointRef<?> endpoint : getEndpoints(objectClazz, method.getDeclaringClass().getSimpleName())) {
