...
public class EndpointRouterTest extends EndpointTestCase {
	...
	public void testBasicRoute() {
		...
		assertRouter(router, SimpleObject.class, RESTActionType.SHOW, 1l);

		router = EndpointRouter.generateRouteFor(r, "POST", "/simpleobjects/1");
		assertRouter(router, SimpleObject.class, RESTActionType.CREATE, 1l);

		router = EndpointRouter.generateRouteFor(r, "PUT", "/simpleobjects/1");
		assertRouter(router, SimpleObject.class, RESTActionType.UPDATE, 1l);

		router = EndpointRouter.generateRouteFor(r, "DELETE", "/simpleobjects/1");
		assertRouter(router, SimpleObject.class, RESTActionType.DELETE, 1l);
	}

	@Test
	public void testCollectionRoute() {
		EndpointRouter router = EndpointRouter.generateRouteFor(r, "POST", "/simpleobjects");
		assertRouter(router, SimpleObject.class, RESTActionType.CREATE, null);
	}
	...
	public void testCustomActionRoute() {
		...
		assertRouter(router, SimpleObject.class, RESTActionType.CUSTOM, 1l);
		assertEquals("active", router.getCustomAction());
	}

	private void assertRouter(EndpointRouter router, Class<SimpleObject> clazz, RESTActionType restAction, Long id) {
