...
public class QueryBuilder<T> {
	...
	private List<IdRef<T>> getForcedResultQuery() {
		return (List<IdRef<T>>) getForcedResult(QueryType.QUERY);
	}
	...
	private List<T> executeQuery() {
		...
		List<T> objects = ids.stream().map(IdRef::fetch).collect(toList());
	...
	public List<IdRef<T>> ids() {
		...
		return doIdsIgnoringPostFilterAndOrder();
	}

	private List<IdRef<T>> doIdsIgnoringPostFilterAndOrder() {
		r.namespace().set(getClazz());
		try {
			executedQueryType = QueryType.QUERY;
			RepositoryHooks.beforeQuery(this);
			List<IdRef<T>> ids = hasForcedResponse(executedQueryType) ? getForcedResultQuery() : doFetchIds();
